...
static void handleLine(const String &line, bool viaUsb)
{
    ...
    {
        sendReply(digitalRead(SIGNAL_PIN) ? "STATE|GREEN" : "STATE|RED", viaUsb);
    }
    else if (line.startsWith("v1|"))
    {
        ...
        if (messageType == "SIGNAL" && desiredState == "GREEN")
        {
            digitalWrite(SIGNAL_PIN, HIGH);
            sendReply(buildAckFrame(NODE_ID, commandId, "ACCEPTED"), viaUsb);
        }
        else if (messageType == "SIGNAL" && desiredState == "RED")
        {
...
void setup()
{
    ...
    Serial.begin(19200);
    Serial.print("Signal node started: ");
